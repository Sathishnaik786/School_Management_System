...
export const ExamEvaluationController = {
    /**
     * Lock Results for a specific subject schedule
     * Prevents further marks entry.
     */
    async lockSubject(req: Request, res: Response) {
        ...
        try {
            ...
            // 2. Atomic RPC call
            const { error } = await supabase.rpc('fn_lock_exam_subject', {
     ...
     */
    async unlockSubject(req: Request, res: Response) {
        const userId = req.context!.user.id;
        const { reason } = req.body;
        const { id } = req.params;

        if (!reason) return res.status(400).json({ error: "Reason required for unlock override." });
