...
import { ExamBootstrapService } from '../services/examBootstrap.service';
...
export const ExamEligibilityController = {
    ...
    async freezeEligibility(req: Request, res: Response) {
        ...
        try {
            ...
            if (studentIds.length === 0) return res.status(400).json({ error: "No students found for this exam's classes." });

            // 3. Compute Live Bulk Eligibility
            const eligibilityMap = await ExamEligibilityService.checkEligibilityBulk(studentIds, examId);
            ...
            // 5. Atomic RPC Freeze
            const { error: freezeErr } = await supabase.rpc('fn_freeze_exam_eligibility', {
                p_exam_id: examId,
    ...
    async overrideEligibility(req: Request, res: Response) {
        const userId = req.context?.user?.id;
        try {
            ...
            const { error } = await supabase.rpc('fn_override_eligibility', {
    ...
    async bootstrapEligibility(req: Request, res: Response) {
        try {
            const { examId } = req.body;
            const userId = req.context?.user?.id;
            ...
            const result = await ExamBootstrapService.bootstrapExam(examId, userId);
            res.json(result);
        } catch (err: any) {
    ...
    async getClassEligibility(req: Request, res: Response) {
        try {
            ...
            if (!sections || sections.length === 0) return res.json({ data: [], total: 0 });
            ...
            const activeEnrollments = (enrollments || []).filter((e: any) => e.student && e.student.status === 'active');
            const studentIds = activeEnrollments.map((e: any) => e.student_id);

            if (studentIds.length === 0) return res.json({ data: [], total: 0 });
            ...
            let processedData = activeEnrollments.map((e: any) => ({
                id: e.student.id,
                full_name: e.student.full_name,
                student_code: e.student.student_code,
                section_name: (e.section as any)?.name || 'N/A',
            ...
            res.json({
                ...
                meta: {
                    total,
                    eligibleCount: activeEnrollments.filter((e: any) => eligibilityMap[e.student_id]?.eligible).length,
                    page: p,
                    pageSize: ps,
