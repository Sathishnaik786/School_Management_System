...
export const ExamHallTicketService = {
    ...
    async generateHallTickets(examId: string, userId: string, schoolId: string) {
        ...
        const { error } = await supabase.rpc('fn_generate_hall_tickets', {
            p_exam_id: examId,
        ...
        if (error) {
            ...
            if (error.message?.includes('SEATING_NOT_PUBLISHED')) throw new Error("SEATING_NOT_PUBLISHED: Please publish seating allocation first.");
            if (error.message?.includes('HALL_TICKETS_LOCKED')) throw new Error("HALL_TICKETS_LOCKED: Cannot regenerate after publication.");
            if (error.message?.includes('NO_SEATING_FOUND')) throw new Error("NO_SEATING_FOUND: No students found in seating allocation.");
            throw error;
        }
     ...
     */
    async publishHallTickets(examId: string, userId: string) {
        const { error } = await supabase.rpc('fn_publish_hall_tickets', {
            p_exam_id: examId,
            p_user_id: userId
        });
        ...
        if (error) {
            ...
            if (error.message?.includes('TICKETS_NOT_READY')) throw new Error("TICKETS_NOT_READY: Generate hall tickets before publishing.");
            throw error;
        }
     ...
     */
    async generateHallTicketPDF(examId: string, studentId: string, schoolId: string): Promise<Buffer> {
        ...
        try {
            ...
            if (exam.seating_status !== 'PUBLISHED') throw new Error("SEATING_NOT_PUBLISHED");
            if (exam.hall_ticket_status !== 'PUBLISHED') throw new Error("HALL_TICKET_NOT_PUBLISHED");
            ...
            const { data: seating, error: seatError } = await supabase
            ...
            if (seatError || !seating) throw new Error("SEATING_NOT_FOUND");
    ...
    async generateBulkHallTicketsZip(examId: string, schoolId: string): Promise<Buffer> {
        ...
        if (ticketError || !tickets || tickets.length === 0) throw new Error("NO_TICKETS_FOUND");

        // 2. Data Aggregation for Template (Shared across all tickets for efficiency)
        const { data: exam } = await supabase.from('exams').select('*, school:school_id(*)').eq('id', examId).single();
        const { data: schedules } = await supabase.from('exam_schedules').select('*, subject:subject_id(name, code)').eq('exam_id', examId).order('exam_date', { ascending: true });
            ...
            try {
                ...
                for (const t of tickets) {
                    ...
                    try {
                        ...
                        const studentCode = (t.student as any).student_code;

                        // Fetch specific seating for this student
                        const { data: seating } = await supabase.from('exam_seating_allocations').select('*, hall:hall_id(*)').eq('exam_id', examId).eq('student_id', studentId).single();
                        const { data: studentDetails } = await supabase.from('students').select('*, admission:admission_id(father_name), sections:student_sections(section:section_id(name, class:class_id(name)))').eq('id', studentId).single();

                        if (!seating || !studentDetails) continue;
                        ...
                        let rows = '';
                        schedules?.forEach((s: any) => { rows += `<tr><td>${s.subject.name}</td><td>${s.exam_date}</td><td>${s.start_time}</td></tr>`; });
