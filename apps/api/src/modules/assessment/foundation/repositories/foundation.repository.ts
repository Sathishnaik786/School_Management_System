...
export class FoundationRepository extends BaseRepository<any> {
    constructor() {
     ...
     */
    public async updateConfig(schoolId: string, payload: any): Promise<any> {
        const { data, error } = await supabase
            .from(this.tableName)
            .update({
                ...payload,
     ...
     */
    public async listWorkflows(schoolId: string): Promise<any[]> {
        const { data, error } = await supabase
     ...
     */
    public async findWorkflowById(workflowId: string, schoolId: string): Promise<any | null> {
     ...
     */
    public async createWorkflow(schoolId: string, payload: any): Promise<any> {
        ...
        if (transitions && transitions.length > 0) {
            const transPayload = transitions.map((trans: any) => ({
                ...trans,
                workflow_id: definition.id
            }));
     ...
     */
    public async updateWorkflow(workflowId: string, schoolId: string, payload: any): Promise<any> {
        const { steps, transitions, ...definitionPayload } = payload;
     ...
     */
    public async deleteWorkflow(workflowId: string, schoolId: string, userId: string): Promise<void> {
