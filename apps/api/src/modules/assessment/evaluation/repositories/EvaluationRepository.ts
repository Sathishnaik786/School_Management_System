...
export class EvaluationRepository extends BaseRepository<any> {
    constructor() {
        super('assessment_evaluation_sessions');
    }

    public async listSessions(schoolId: string, status?: string): Promise<any[]> {
        let query = supabase
            .from(this.tableName)
    ...
    public async findSessionById(sessionId: string, schoolId: string): Promise<any | null> {
        ...
        const { data: questionEvaluations, error: qErr } = await supabase
    ...
    public async updateSessionStatus(sessionId: string, status: string): Promise<any> {
        const { data, error } = await supabase
            .from(this.tableName)
            .update({
                status,
    ...
    public async saveQuestionEvaluation(sessionId: string, payload: any, userId?: string): Promise<any> {
        ...
        if (payload.annotations && payload.annotations.length > 0) {
            ...
            const annPayload = payload.annotations.map((ann: any) => ({
