...
async function main() {
    console.log("Applying migration 123_examination_operations_schema.sql...");
    const sqlPath = path.join(__dirname, '../database/migrations/123_examination_operations_schema.sql');
    if (!fs.existsSync(sqlPath)) {
        throw new Error(`Missing migration file at: ${sqlPath}`);
    }
    const sql = fs.readFileSync(sqlPath, 'utf8');
    
    // Split queries while removing comments and BEGIN/COMMIT blocks
    ...
    console.log("✅ Migration 123 successfully applied!");
}
