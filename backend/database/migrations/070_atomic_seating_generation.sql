-- =======================================================
-- MIGRATION: 070_atomic_seating_generation
-- DESCRIPTION: Implements Atomic Seating Generation via RPC and Hardens Seat Uniqueness.
-- =======================================================

BEGIN;

-- 1. Harden Seat Uniqueness at the Exam Level
-- Ensure exam_id column exists (in case 068 wasn't fully applied)
...
-- Prevents multiple students from being assigned the same seat in the same hall for one exam.
ALTER TABLE public.exam_seating_allocations 
DROP CONSTRAINT IF EXISTS unique_hall_seat_exam;

ALTER TABLE public.exam_seating_allocations 
ADD CONSTRAINT unique_hall_seat_exam UNIQUE (exam_id, hall_id, seat_number);

-- 2. Atomic Seating Generation Function
-- Handles validation, deletion, and allocation in a single transaction.
...
BEGIN
    ...
    FROM public.exam_halls WHERE school_id = p_school_id;

    IF v_eligible_count > v_total_capacity THEN 
        RAISE EXCEPTION 'INSUFFICIENT_CAPACITY: Need % seats, only % available.', v_eligible_count, v_total_capacity; 
    END IF;

    -- D. Clear existing (Atomic within transaction)
    DELETE FROM public.exam_seating_allocations WHERE exam_id = p_exam_id;

    -- E. Setup Hall Iteration
    ...
    -- F. Allocation Loop (Joined with students for code-sorting parity)
