...
export class AdmissionCRMTransactionService extends BaseService {
     ...
     */
    public async convertEnquiryToLead(enquiryId: string, leadId: string, correlationId?: string): Promise<void> {
        ...
        const sqlQueries: string[] = [
            ...
            `INSERT INTO public.admission_leads (id, enquiry_id, status, created_at, updated_at) VALUES ('${leadId}', '${enquiryId}', 'NEW', NOW(), NOW())`,
            `INSERT INTO public.status_history (entity_name, entity_id, old_status, new_status, reason, correlation_id, event_name, created_at) VALUES ('admission_leads', '${leadId}', NULL, 'NEW', 'Lead created from converted enquiry', ${correlationId ? `'${correlationId}'` : 'NULL'}, 'LeadCreated', NOW())`
        ];
