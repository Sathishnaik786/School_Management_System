...
export const ExamEligibilityService = {
    async checkEligibility(studentId: string, examId: string): Promise<{
        ...
        reasons: string[];
        source?: 'REAL' | 'BOOTSTRAP' | 'OVERRIDE';
    }> {
        ...
        if (exam.eligibility_frozen) {
            ...
            if (snapshot) {
                return {
                    ...
                    fees_status: snapshot.fees_status as any,
                    reasons: snapshot.reasons as any[],
        ...
        if (attCache && ['ADMIN', 'BOOTSTRAP', 'SYSTEM'].includes(attCache.source)) {
            ...
            let attendedSessions = records?.filter((r: any) =>
        ...
        if (feeCache && ['ADMIN', 'BOOTSTRAP', 'REAL'].includes(feeCache.source)) {
            ...
            if (!isFeeCleared) reasons.push(`Fees Pending (Source: ${feeCache.source})`);
        } else {
        ...
        if (!isFeeCleared) eligible = false;
        ...
        return {
            ...
            reasons,
            source: (() => {
                if (attCache?.source === 'ADMIN' || feeCache?.source === 'ADMIN') return 'OVERRIDE' as const;
                if (attCache?.source === 'BOOTSTRAP' || feeCache?.source === 'BOOTSTRAP') return 'BOOTSTRAP' as const;
                if (attCache?.source === 'REAL' || feeCache?.source === 'REAL') return 'REAL' as const;
                return undefined;
            })()
        };
    },
     ...
     */
    async checkEligibilityBulk(studentIds: string[], examId: string): Promise<Record<string, {
        eligible: boolean;
        attendance_percentage: number;
        fees_status: 'CLEARED' | 'PENDING';
        reasons: string[];
        ...
        if (exam.eligibility_frozen) {
            ...
            if (snapshots && snapshots.length > 0) {
                ...
                // If all found, return
                if (snapshots.length === studentIds.length) return results;
            }
        }
        ...
        const { data: attCaches } = await supabase
            .from('student_attendance_cache')
            .select('student_id, attendance_percentage, source')
            .eq('academic_year_id', exam.academic_year_id)
            .in('student_id', missingIds);

        const { data: feeCaches } = await supabase
            .from('student_fee_clearance_cache')
        ...
        missingIds.forEach(sid => {
            const reasons: string[] = [];
            let eligible = true;
            let source: 'REAL' | 'BOOTSTRAP' | 'OVERRIDE' | undefined = undefined;
            ...
            if (cacheAtt && ['ADMIN', 'BOOTSTRAP', 'SYSTEM'].includes(cacheAtt.source)) {
                ...
                if (cacheAtt.source === 'ADMIN') source = 'OVERRIDE';
            } else {
                // Live Calc
            ...
            if (cacheFee && ['ADMIN', 'BOOTSTRAP', 'REAL'].includes(cacheFee.source)) {
                ...
                if (!feeCleared) reasons.push(`Fees Pending`);
                if (cacheFee.source === 'ADMIN') source = 'OVERRIDE';
            } else {
                // Live Calc
            ...
            if (!feeCleared) eligible = false;

            // Determine effective source
            if (!source) {
                if (cacheAtt?.source === 'BOOTSTRAP' || cacheFee?.source === 'BOOTSTRAP') source = 'BOOTSTRAP';
                else if (cacheAtt?.source === 'REAL' || cacheFee?.source === 'REAL') source = 'REAL';
            }
