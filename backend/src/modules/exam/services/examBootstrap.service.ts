...
export const ExamBootstrapService = {
    /**
     ...
     */
    async bootstrapExam(examId: string, userId: string) {
        ...
        if (classIds.length === 0) throw new Error("No classes found for this exam context.");

        const { data: enrollments, error: stuError } = await supabase
        ...
        if (!enrollments || enrollments.length === 0) return { count: 0, message: "No students found." };
    ...
    async validateBootstrap(examId: string) {
        const { data: snapshots, count } = await supabase
            .from('exam_eligibility_snapshots')
            .select('source', { count: 'exact' })
            .eq('exam_id', examId);

        const breakdown = snapshots?.reduce((acc: any, curr) => {
