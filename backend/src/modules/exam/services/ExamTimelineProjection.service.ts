...
export const ExamTimelineProjectionService = {
    async getStudentTimeline(studentId: string, schoolId: string) {
        ...
        const classId = (enrollment.section as any).class.id;
        const academicYearId = (enrollment.section as any).class.academic_year_id;
        ...
        const { data: eligSnapshot } = await supabase
        ...
        const { data: seating } = isSeatingPublished ? await supabase
        ...
        const { data: result } = isResultsPublished ? await supabase
    ...
    async getFacultyTimeline(facultyUserId: string, schoolId: string) {
        ...
        if (!assignments || assignments.length === 0) return { examName: 'No Active Exams', examStatus: 'DRAFT', events: [] };

        const classIds = Array.from(new Set(assignments.map((a: any) => a.section.class.id)));
        const academicYearId = (assignments[0] as any).section.class.academic_year_id;
        ...
        const relevantSchedules = schedules?.filter(s => classIds.includes((s.subject as any).class_id)) || [];
