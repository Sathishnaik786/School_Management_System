...
export class QuestionVersionRepository extends BaseRepository<any> {
    constructor() {
        super('assessment_question_bank');
    }

    public async findVersions(questionId: string, schoolId: string): Promise<any[]> {
    ...
    public async restoreVersion(questionId: string, versionNumber: number, schoolId: string, userId: string): Promise<any> {
        // Find the past version row
        const { data: pastVer, error: vError } = await supabase
            .from(this.tableName)
            .select('*')
            .eq('school_id', schoolId)
        ...
        if (!pastVer) throw new Error(`Version ${versionNumber} not found.`);
        ...
        if (!live) throw new Error(`Live question not found.`);
...
export default QuestionVersionRepository;
