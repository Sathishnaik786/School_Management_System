...
export class TemplatePreviewEngine extends BaseService {
    private readonly repo = new TemplateRepository();
    private readonly cacheRepo = new TemplatePreviewCacheRepository();

    public async generatePreview(templateId: string, format: string, schoolId: string, correlationId?: string): Promise<any> {
        ...
        const font = template.layoutRules?.find((r: any) => r.property === 'font')?.value || 'Arial';
        const orientation = template.layoutRules?.find((r: any) => r.property === 'orientation')?.value || 'Portrait';
        const columns = template.layoutRules?.find((r: any) => r.property === 'columns')?.value || '1';
